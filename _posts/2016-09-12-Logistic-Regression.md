...
Instead of trying to come up with a function $$ y = f(X) $$ which takes the values 0 and 1, we can try to approximate the probability function $$ p(y \vert X) $$. Then we may assign $$y$$ the value of 1 if we estimate a probability higher than .5, and the value 0 otherwise.
